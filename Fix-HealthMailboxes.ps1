...
Param(
    [parameter(Mandatory,HelpMessage='Remove HealthMailboxes in a corrupted state',ParameterSetName='R')]
    [switch]$Remove,
    [parameter(Mandatory,HelpMessage='Disable HealthMailboxes in a corrupted state',ParameterSetName='D')]
    [switch]$Disable
)
...
function script:CheckHealthMailboxes()
{
    ...
    Write-Output ('{0} HealthMailbox(es) found' -f $count) 

    $i=0
    
    foreach($mailbox in $healthMailboxes) {
        try {
            Write-Output ('Checking: {0}' -f $mailbox.UserPrincipalName)
            
            If(($mailbox.database -eq '') -or ($mailbox.database -eq $null)) {
                $upn = $mailbox.UserPrincipalName

                Write-Warning ('Database attribute check failed: {0}' -f $upn) 
                if($i -lt 50) {
                    try {
                        Write-Output ('Disabling mailbox {0}' -f $upn)
                        $i++
            ...
            $upn = $mailbox.UserPrincipalName
            Write-Warning ('Mailbox with warning: {0}' -f $upn)
        }
    }
    Write-Output ('{0} HealthMailbox(es) deleted' -f $i)
}
